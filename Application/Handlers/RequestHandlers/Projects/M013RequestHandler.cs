...
public class M013RequestHandler : IRequestHandler<M013Request, IResult>
{
    private readonly IFileStorageService _storageService;
    private readonly IRepository<Project> _repository;
    private readonly ReleaseOptions _releaseOptions;

    public M013RequestHandler(IFileStorageService storageService, IRepository<Project> repository, IOptions<ReleaseOptions> options)
    {
        _storageService = storageService;
        _repository = repository;
        _releaseOptions = options.Value;
    }
    public async Task<IResult> Handle(M013Request request, CancellationToken cancellationToken)
    {
        try
        {
